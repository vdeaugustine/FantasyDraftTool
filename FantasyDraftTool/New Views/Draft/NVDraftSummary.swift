...
// MARK: - SortTeamsOptions

enum SortTeamsOptions {
    case points, score
}

// MARK: - NVDraftSummaryView
            ...
            ForEach(model.draft.teams, id: \.self) { team in
                Section(team.name) {
                    ForEach(Position.batters, id: \.self) { position in
                        ...
                        Section(position.str) {
                            ForEach(team.players(for: position), id: \.self) { player in
...
//        }
        .listStyle(.plain)
        .onAppear {
            model.draft.simulatePicks(80, projection: .depthCharts) { draft in
                model.draft = draft
            }
        }
...
// MARK: - NVDraftSummaryView_Previews
